...
updated : 2020-09-05 21:21:26 +0900
tags    : 
parent  : 
banner  : './thumb.png'
---

![유니버셜링크 썸네일](./thumb.png)
...
먼저 짚고 넘어거자.유니버셜 링크라고 이야기하지만, 유니버셜 링크는 iOS 에서만 작동하는 딥링크의 한 종류이다. 
그럼 딥링크를 생각해보아야할텐데, 딥링크는 뭘까? 일반적인 딥링크는 하나의 웹 링크를 눌렀을 때 
그 서비스에 해당하는 앱이 있다면 -> 앱으로, 
앱이 없다면
...
딥링크의 형태는 앱 스킴(URL 스킴) 혹은 일반적인 `https://` 링크 두 가지 형태를 둘다 지원한다. 

모바일 시대를 살고 있으므로, 이 기능을 아주 많이 쓰게 된다. 예를 들어, 새로운 게임 출시 사전등록을 했다고 하자. 
출시 당일이 되면 링크 하나가 문자로 날라오는데, 이 경우 웹페이지 하나가 떴다가 스토어로 자동으로 이동한다. 
앱을 설치한 이후에 이 링크를 한번 더 누르면, 바로 게임이 열린다. 🙆‍♀️ 이때 우리는 순간적으로 지나갔지만 
딥링크를 사용한 것이다. 
