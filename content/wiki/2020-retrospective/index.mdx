...
updated : 2020-12-21 08:31:45 +0900
tags    : 
- Postmortem
banner  : './thumb.png'
---
...
블로그 배포 플랫폼을 netlify에서 vercel로 옮겼다.
...
알림도메인을 한다는 것... 그것은 알림 스펙도 계속 만든다는 것... 알림 스펙 구현을 위해서 spring batch + kafka  + kafka consumer 조합으로 알림을 보내는 접근법을 사용했다. 아직도 kafka를 잘모르는데 컨슈머를 작성할 수 있다니!

그외에 라인 로그인 연동은 나의 담당이기때문에.. 라인 탈퇴 시 서비스 탈퇴에 대한 처리를 했다. 다른 서비스는 어떻게 하고 있지? 😪 
...
QA 와 서비스 리얼 배포준비로 분주했다. QA 테스트로 값을 올렸다 내렸다 조정하고, 누락된 기획을 적용하느라 바빴다. 또 real 서비스와 연동하고 QA에서 나온 버그를 수정했다. 지금은 내부 사정으로 서비스가 밀려서 다시 한번 조금 스트레스를 받고 있는 실정이다. =) 
...
이 회고를 12월 까지 다 써놨었는데, 한번 날아갔다. 중간에 한번도 커밋하지 않은 내 자신.. 조금 슬프다. 하루 자고 나니까 그 글은 조금 번잡했지? 싶네. 이건 좀더 깨끗한 버전이다. 😗  주말동안 회고를 다시 썼다. 힘내라 2021년도!

<blockquote class="twitter-tweet"><p lang="ko" dir="ltr">회고 한바닥 썼는데, github action 실행하는 순간 내가 이걸 덮어씌워지는 결과물 파일에 작성했다는걸 깨달았다. 빈 md 파일에서 빈 결과물이 생성되었고.. 😇</p>&mdash; 멍준꿀 (@juneyr_) <a href="https://twitter.com/juneyr_/status/1339867465724006400?ref_src=twsrc%5Etfw">December 18, 2020</a></blockquote> 
