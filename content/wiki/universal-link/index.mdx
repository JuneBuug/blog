...
딥링크의 형태는 앱 스킴(URL 스킴) 혹은 일반적인 `https://` 링크 두 가지 형태를 둘다 지원한다. 즉 `https://www.example.com` 혹은 `app://open.my.app` 두 형태를 모두 지원한다는 말이다. 또한, 아래 그림처럼 눌렀을 때 필요한 앱을 직접 선택할 수 있도록 한다는 특징이 있다.
