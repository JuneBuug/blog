...
단조 읽기(monotonic read)는 이런 이상현상이 발생하지 않음을 보장한다. 단조 읽기는 강한 일관성보다는 덜하지만, 최종적 일관성보다는 더 강한 보장이다. 한 사용자는 새로운 데이터를 읽으면 예전 데이터를 읽지않는다. 
...
#### 복제 지연을 위한 해결책 
이런 식으로 복제 지연이 일어나면서 일어나는 문제들이 있다.이런 식으로 복제 지연이 일어나면서 일어나는 문제들이 있다.  
